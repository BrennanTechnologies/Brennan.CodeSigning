...
function Get-ABACodeSigningCerts {
    ...
    foreach($CertStoreLocation in $CertStoreLocations){
        try {
            $CodeSigningCerts += @( ((Get-ChildItem $CertStoreLocation -codesigning -Recurse) | Where {$_.Subject -match "CN=Abacus Group LLC"} | Select-Object -Property *) )
        } catch {
    ...
    if( $CodeSigningCerts.Count -eq 0 ) {
        Write-Host "No Abacus Code Signing Certs where found on this computer."
    }
    else {
